...
namespace settings {
  ...
  enum class CELL_DATA_ORDER {
    ...
    cell_refs,
    flush_byte,
    extra_prunned_branch,
  };
  static const std::vector<std::vector<enum CELL_DATA_ORDER>> save_data_order = {
    {CELL_DATA_ORDER::d2,CELL_DATA_ORDER::d1,CELL_DATA_ORDER::special_cell_type,CELL_DATA_ORDER::cell_refs,CELL_DATA_ORDER::flush_byte,},
    // {CELL_DATA_ORDER::extra_prunned_branch, CELL_DATA_ORDER::flush_byte},
...
td::Result<std::size_t> CustomBagOfCells::serialize_to_impl(WriterT& writer) {
  ...
  for (auto stored_data : settings::save_data_order) {
    for (int i = cell_count - 1; i >= 0; --i) {
      ...
      auto store_cell_data = [&](bool is_special) {
        for (int i = 2 + int(is_special); i < s; ++i) {
...
struct LoadCellData {
  int d1 = -1, d2 = -1, special_cell_type = -1;
      ...
      for (auto mode : stored_data) {
        switch (mode) {
          ...
          case settings::CELL_DATA_ORDER::cell_data: {
            ...
            TRY_STATUS(cell_info.init());
            TRY_STATUS(cell_info.init_data());
            for (int i = cell_info.special; i < cell_info.data_len; ++i) {
